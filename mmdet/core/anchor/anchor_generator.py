...
class AnchorGenerator:
    ...
    def valid_flags(self, featmap_sizes, pad_shape, img_meta, device='cuda'):
    ...
    def single_level_valid_flags(self,
        ...
        image = cv2.imread(img_meta['filename'], 0)
        thresh, binary = cv2.threshold(image, 0, 255, cv2.THRESH_OTSU)
        kernel = cv2.getStructuringElement(0, (12, 12))
        dst = cv2.erode(binary, kernel)
        result = cv2.resize(dst, (feat_w, feat_h))
        ...
        result = torch.tensor(result, dtype=bool)
        valid = result[:, None].expand(result.size(0),
